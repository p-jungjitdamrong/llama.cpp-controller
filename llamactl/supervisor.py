...
def router_section_name(path: Path, models_dir: Path) -> str | None:
    """What a router calls this model, or None if it would not see it at all.

    A file inside --models-dir is named by its stem. A file in the Hugging Face
    cache is named `org/repo:QUANT`, because that is how llama.cpp's downloader
    knows it — matching that is the only way a preset section can reach it.
    """
    if path.parent == models_dir:
        return path.stem

    from .models import _hf_repo
    ...
    if not quant:
        from .models import _guess_from_name
        quant = _guess_from_name(path.name).get("quant_guess")
    return f"{repo}:{quant}" if quant else None
...
class ServerInstance:
    ...
    @staticmethod
    def _preset_lines(params: LaunchParams, shared: LaunchParams | None = None) -> list[str]:
        """INI keys for one model — long flag names without the leading dashes.

        With `shared` given, only the values that differ from it are written, so a
        per-model section stays short and inherits the rest from `[*]`.
        """
        candidates = [
            ("gpu-layers", params.n_gpu_layers, shared and shared.n_gpu_layers),
            ("ctx-size", params.ctx_size, shared and shared.ctx_size),
            ("parallel", params.parallel, shared and shared.parallel),
            ("jinja", 1 if params.jinja else 0, shared and (1 if shared.jinja else 0)),
            ("mmap", 0 if params.no_mmap else 1, shared and (0 if shared.no_mmap else 1)),
            ("threads", params.threads or None, shared and (shared.threads or None)),
            ("batch-size", params.batch_size or None, shared and (shared.batch_size or None)),
            ("flash-attn", params.flash_attn or None, shared and (shared.flash_attn or None)),
            ("mlock", 1 if params.mlock else None, shared and (1 if shared.mlock else None)),
        ]
        return [f"{key} = {value}" for key, value, inherited in candidates
                if value is not None and (shared is None or value != inherited)]

    def write_router_preset(self) -> Path:
        """Write the INI the router applies to the models it loads.

        `[*]` carries the launch params from the panel. Any model that has been
        started on its own gets a section of its own with whatever it does
        differently — so a model tuned in "One model" mode keeps that tuning when
        the router loads it.
        """
        lines = ["# generated by llama-controller — edit the launch params in the UI",
                 "", "[*]", *self._preset_lines(self.params)]

        # Several files can share one router name — the cache often holds both a
        # single-file and a sharded copy of the same quantisation — and a repeated
        # section would silently shadow itself, so collisions collapse to one and
        # are written down rather than hidden.
        directory = Path(self.params.models_dir).expanduser()
        sections: dict[str, list[str]] = {}
        collisions: dict[str, list[str]] = {}
        for model_path, saved in self.cfg.presets.items():
            path = Path(model_path)
            if not path.exists():
                continue
            section = router_section_name(path, directory)
            if section is None:
                continue  # the router has no name for this model
            per_model = self._preset_lines(LaunchParams.from_dict(saved), self.params)
            if not per_model:
                continue
            if section in sections:
                collisions.setdefault(section, []).append(path.name)
            sections[section] = per_model

        for section, body in sections.items():
            if section in collisions:
                lines += ["", f"# also matched by: {', '.join(collisions[section])}"]
            lines += ["", f"[{section}]", *body]

        target = CONFIG_PATH.parent / f"router-preset-{self.params.port}.ini"
        target.write_text("\n".join(lines) + "\n")
        return target
