...
class Tuner:
    ...
    def start(self, model_path: str, port: int,
              ...
              ctx_size: int | None = None) -> BenchRun:
        if self.busy:
        ...
        self.run = run
        run.task = asyncio.create_task(self._sweep(run, base, options, port))
        return run
    ...
    async def _sweep(self, run: BenchRun, base: LaunchParams,
                     options: list[dict[str, Any]], port: int) -> None:
        ...
        try:
            ...
            ranked = [r for r in run.results if r.get("generate_tps")]
            best = max(ranked, key=lambda r: r["generate_tps"]) if ranked else None
